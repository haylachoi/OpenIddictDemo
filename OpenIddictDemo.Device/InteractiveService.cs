...
namespace OpenIddictDemo.Device;
...
public class InteractiveService : BackgroundService
{
    ...
    public InteractiveService(IHostApplicationLifetime lifetime, OpenIddictClientService service)
    {
        _lifetime = lifetime;
        _service = service;
    }
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        try
        {
            ...
            var result = await _service.ChallengeUsingDeviceAsync(
                new() { CancellationToken = stoppingToken }
            );
            ...
            var principal = (
                await _service.AuthenticateWithDeviceAsync(
                    new()
                    {
                        ...
                        Timeout =
                            result.ExpiresIn < TimeSpan.FromMinutes(5)
                                ? result.ExpiresIn
        ...
        catch
        {
            AnsiConsole.MarkupLine(
                "[red]An error occurred while trying to authenticate the user.[/]"
            );
        }
    }
}
